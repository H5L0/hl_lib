	...
	const char *format, const char *types, void *args_list)
{
	...
	void *args_ptr[_args_temp_size];  //ptr table is unnessary in x64 machine.
	...
	{
		args_ptr[i] = args_list;
		args_list += sizeof(ureg);  //size of arg, (all are 8 byte width in x64)
	}
...
int hlcsWriteA(const char *format, const char *types, ...)
{
	if(format == NULL || *format == '\0') return 0;
	if(types == NULL) return hl_file_write(0, format, hlGetCharsLength(format) + 1);
	...
	int count = hl_string_format_vargs(buffer, buffer + 4096, format, types, vlist);
